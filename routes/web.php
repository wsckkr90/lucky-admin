...
Route::get('/chart/{game}/{year}/{month}', [ChartController::class, 'index'])
    ->where('game', '[a-zA-Z0-9-]+')
    ->where('year', '[0-9]{4}')
    ->where('month', '0?[1-9]|1[0-2]')
    ->name('chart.month');

Route::get('/chart/{game}/{year?}', [ChartController::class, 'index'])
    ->where('game', '[a-zA-Z0-9-]+')
    ->where('year', '[0-9]{4}')
    ->name('chart.game');
...
Route::middleware(['auth', 'admin'])
    ...
    ->group(function () {
        ...
        Route::resource('results', GameResultController::class);
